...
const HOPPLA_EJS_FILE_SUFFIX_REGEX = new RegExp(`\.${util.escapeRegExp(HOPPLA_EJS_FILE_SUFFIX)}$`)
...
const copyToTmpRecursive = function(options) {
  ...
  const relativeDestination = path.relative(templateTmpDir, destinationAndSource);
...
var hoppla = function(options) {
  ...
  return Promise.resolve()
    ...
    .then(() => {
      console.log(`Preparing files from template: ${Config.template.get(config)}`)
  ...
  if (isDirectory) {
    ...
    if (isRawDirectory) {
      ...
      if (fs.existsSync(destinationAndSource)) {
        if (!force) {
          console.warn(`Folder already exists: "${relativeDestinationAndSource}"`);
          return;
        }
      ...
      shell.mkdir('-p', destinationAndSource);
      shell.cp('-R', path.join(source, '*'), destinationAndSource);
    ...
    .then(() => {
      ...
      if (hopplaConfig.raw != null) {
        ...
        if (!targetFileInfo.isDirectory) {
          ...
          targetFileInfo.raw = micromatch([targetFileInfo.pathOrigRelative], [
            '**/*',
            '!**/*hopplaconfig',
            `!**/*.${HOPPLA_EJS_FILE_SUFFIX}`,
          ]).length > 0;
        }
    ...
    .then(() => {
      ...
      if (!hopplaConfig.fileName) {
        if (targetFileInfo.path.match(HOPPLA_EJS_FILE_SUFFIX_REGEX)) {
          hopplaConfig.fileName = targetFileInfo.path.replace(HOPPLA_EJS_FILE_SUFFIX_REGEX, '');
        }
      }
      ...
      if (hopplaConfig.fileName) {
        ...
        if (targetOld !== targetFileInfo.path) {
          if (targetFileInfo.isDirectory) {
            // Directories will be merged, if the new path already exists
            shell.mkdir('-p', targetFileInfo.path);
            shell.cp('-r', path.join(targetOld, '*'), targetFileInfo.path);
          } else {
...
const applyTransformationDirectory = function({ config, rootConfig, targetFileInfo }) {
  return aPromise
    .resolve()
    .then(() => {
      ...
      children.forEach(fileName => {
        ...
        const hopplaConfigPath = hopplaConfigByFileName[fileName.replace(HOPPLA_EJS_FILE_SUFFIX_REGEX, '')];
