    ...
    .then(() => {
      ...
      if (targetFileInfo.isDirectory) {
        ...
        try {
          ...
          util.chmod(targetFileInfo.octalPermissions, targetFileInfo.path);
        } catch (err) {
          console.error(err.message);
...
/**
 * Export slightly simplified copyRecursive api for external use
 */
const copyRecursive = function({
  source,
  destination,
  force = false,
  silent = false,
  ...
  const destinationParent = path.resolve(destination, "..");

  return util.copyRecursive({
    source,
    destination,
    force,
    outputProgress: !silent,
    templateTmpDir: destinationParent,
    rootDestination: baseDir || destinationParent,
